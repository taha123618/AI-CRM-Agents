...
DEFAULT_STARTER_WORKFLOWS = [
    {
        ...
        "nodes": [
            ...
            },
            {
                "id": "node-3",
                "type": "agent",
                "label": "AI SDR Cadence Step 1 (Email)",
            ...
            },
            {
                "id": "node-4",
                "type": "agent",
                "label": "WhatsApp Auto-Pilot Intro",
            ...
            },
            {
                "id": "node-5",
                "type": "action",
                "label": "Create Pipeline Opportunity",
        ...
        "nodes": [
            {
                "id": "n-1",
                "type": "trigger",
                "label": "Stage Changed to Proposal",
                "position": {"x": 50, "y": 100},
            },
            {
                "id": "n-2",
                "type": "agent",
                "label": "Sales Pipeline & War Room Consensus",
            ...
            },
            {
                "id": "n-3",
                "type": "action",
                "label": "Generate Smart Proposal SLA",
...
class WorkflowCreateRequest(BaseModel):
    ...
    trigger_type: str = Field(
        "event", description="'event', 'manual', 'schedule', 'webhook'"
    )
...
def list_workflows(
    ...
    workflows = (
        db.query(WorkflowDefinition).order_by(desc(WorkflowDefinition.created_at)).all()
    )
    ...
    return [
        WorkflowResponse(
            ...
            last_executed_at=w.last_executed_at.isoformat()
            if w.last_executed_at
            else None,
...
def create_workflow(
    ...
    return WorkflowResponse(
        ...
        last_executed_at=new_wf.last_executed_at.isoformat()
        if new_wf.last_executed_at
        else None,
...
def get_workflow(
    ...
    return WorkflowResponse(
        ...
        last_executed_at=wf.last_executed_at.isoformat()
        if wf.last_executed_at
        else None,
...
def delete_workflow(
    ...
    return {
        "status": "success",
        "message": f"Workflow '{wf.name}' deleted successfully.",
    }

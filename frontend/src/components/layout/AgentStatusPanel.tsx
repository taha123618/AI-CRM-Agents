    ...
    <div className="bg-[#1F2833] rounded-none p-4 border border-[#3A4552] mb-6 relative overflow-hidden font-mono">
        ...
        <div className="flex items-center gap-2">
          {onRefresh && (
            <Button variant="outline" size="sm" onClick={onRefresh} className="text-xs h-7">
              <RefreshCw className="w-3 h-3 text-[#FFB800] mr-1" />
              <span>REFRESH</span>
            </Button>
          )}
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-none bg-[#0B0C10] text-[#FFB800] border border-[#FFB800] text-[10px] font-bold uppercase shrink-0">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 relative z-10">
        {agentsList.map((agent) => (
          <div
            key={agent.name}
            className="p-2.5 rounded-none bg-[#0B0C10] border border-[#3A4552] flex flex-col justify-between hover:border-[#FFB800] transition-none group"
          >
            ...
            <p className="text-[11px] font-bold text-slate-200 mt-2 truncate group-hover:text-[#FFB800] transition-none uppercase" title={agent.name}>
              {agent.name.replace('Agent', '')}
            </p>
            <span className="text-[8px] text-[#FFB800] font-bold uppercase tracking-wider mt-1 block">ACTIVE</span>
          </div>
        ))}
      </div>
    </div>

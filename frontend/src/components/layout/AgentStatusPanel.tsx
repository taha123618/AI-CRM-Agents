    ...
    <div className="bg-[#1F2833] rounded-none p-4 border border-[#3A4552] mb-6 relative overflow-hidden font-mono">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between pb-3 border-b border-[#3A4552] mb-3 relative z-10 gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <span className="text-[9px] text-slate-400 uppercase">MULTI-AGENT ORCHESTRATOR DOCK</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {onRefresh && (
            <Button variant="outline" size="sm" onClick={onRefresh} className="text-xs h-7">
              <RefreshCw className="w-3 h-3 text-[#39FF14] mr-1" />
              <span>REFRESH</span>
            </Button>
          )}
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-none bg-[#0B0C10] text-[#39FF14] border border-[#39FF14] text-[10px] font-bold uppercase shrink-0">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 relative z-10">
        {agentsList.map((agent) => (
          <div
            key={agent.name}
            className="p-2.5 rounded-none bg-[#0B0C10] border border-[#3A4552] flex flex-col justify-between hover:border-[#39FF14] transition-none group"
          >
            <div className="flex items-center justify-between">
              <span className="relative flex h-2 w-2">
                <span className="relative inline-flex rounded-none h-2 w-2 bg-[#39FF14]"></span>
              </span>
            ...
            <p className="text-[11px] font-bold text-slate-200 mt-2 truncate group-hover:text-[#39FF14] transition-none uppercase" title={agent.name}>
              {agent.name.replace('Agent', '')}
            </p>
            <span className="text-[8px] text-[#39FF14] font-bold uppercase tracking-wider mt-1 block">ACTIVE</span>
          </div>
        ))}
      </div>
    </div>

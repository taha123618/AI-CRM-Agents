...
export const forecastingApi = {
  runMonteCarlo: async (payload: {
    ...
    const { data } = await apiClient.post<MonteCarloResult>('/api/forecasting/monte-carlo', payload);
    return data;
  },

  getPipelineVelocity: async (): Promise<PipelineVelocityMatrix> => {
    const { data } = await apiClient.get<PipelineVelocityMatrix>('/api/forecasting/pipeline-velocity');
    return data;
  },
  ...
  getSavedSimulations: async (): Promise<ForecastSimulationRecord[]> => {
    const { data } = await apiClient.get<ForecastSimulationRecord[]>('/api/forecasting/simulations');
    return data;
  },
};

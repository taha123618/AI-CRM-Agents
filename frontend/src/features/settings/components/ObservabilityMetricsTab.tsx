    ...
    <div className="space-y-4 font-mono">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 rounded-none bg-[#1F2833] border border-[#3A4552]">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <RefreshCw className={`w-3 h-3 ${isRefetching ? 'animate-spin text-[#39FF14]' : ''}`} />
            <span>REFRESH</span>
          </Button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
        <div className="p-3.5 rounded-none bg-[#1F2833] border border-[#3A4552] space-y-1">
          ...
          <span className="text-[9px] text-[#39FF14] uppercase">REAL-TIME BROADCAST CHANNELS</span>
        </div>
        ...
        <div className="p-3.5 rounded-none bg-[#1F2833] border border-[#3A4552] space-y-1">
          ...
          <span className="text-[9px] text-[#39FF14] uppercase">AUTONOMOUS ORCHESTRATIONS</span>
        </div>
        ...
        <div className="p-3 bg-[#1F2833] border-b border-[#3A4552] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="bg-[#0B0C10] border border-[#3A4552] rounded-none px-2.5 py-1 text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-[#39FF14] uppercase font-mono w-56"
            />
            {filterText && (
            ...
            filteredLines.map((line, idx) => {
              ...
              return (
                <div key={idx} className="flex items-baseline justify-between hover:bg-[#1F2833] px-1 py-0.5 transition-none">
                  ...
                  <span className="text-[#39FF14] font-bold">{value}</span>
                </div>
              );
            })
          )}
